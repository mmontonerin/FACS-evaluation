...
os.makedirs("plots_separate_species", exist_ok=True)
...
# Perform log normalization for each sample ID separately
for sample_id, sp_group in df_filtered.groupby('Sample ID'):
    sp_group = log_normalize(sp_group, columns_to_plot)
    ...
    for column_x, column_y in [('488-SSC-Height-Log-sep', '488-530/40-Height-Log-sep'),
        ...
        mean_x = sp_group[column_x].mean()
        std_x = sp_group[column_x].std()
        mean_y = sp_group[column_y].mean()
        std_y = sp_group[column_y].std()

        legend_handles = []

        for pcr, marker in pcr_markers.items():
            pcr_group = sp_group[sp_group[pcr] == True]
            if not pcr_group.empty:
                # Get the family of the first row in the group
                family = pcr_group.iloc[0]['Family']
                ...
                # Pearson correlation coefficient if there are enough data points
                if len(pcr_group) >= 2:
        ...
        # Statistical significance annotation fungi vs negative
        if 'pcr_f' in sp_group.columns and 'pcr_n' in sp_group.columns:
            pcr_f_values = sp_group[sp_group['pcr_f'] == True][[column_x, column_y]].values.flatten()
            pcr_n_values = sp_group[sp_group['pcr_n'] == True][[column_x, column_y]].values.flatten()
        ...
        if 'pcr_f' in sp_group.columns and 'pcr_b' in sp_group.columns:
            pcr_f_values = sp_group[sp_group['pcr_f'] == True][[column_x, column_y]].values.flatten()
            pcr_b_values = sp_group[sp_group['pcr_b'] == True][[column_x, column_y]].values.flatten()
        ...
        plt.savefig(f"plots_separate_species/{sample_id}_{valid_filename}.pdf", bbox_inches='tight', bbox_extra_artists=[legend_pcr])
        plt.savefig(f"plots_separate_species/{sample_id}_{valid_filename}.png", bbox_inches='tight', bbox_extra_artists=[legend_pcr])
        plt.close()

    # Standardize the data
    x = sp_group[columns_to_plot].values
    ...
    principal_df = pd.concat([principal_df, sp_group[['Family'] + list(pcr_markers.keys())].reset_index(drop=True)], axis=1)
    ...
    plt.title(f'PCA of coordinates - {sample_id}')
    ...
    plt.savefig(f"plots_separate_species/{sample_id}_pca_plot.pdf", bbox_inches='tight', bbox_extra_artists=[legend_pcr])
    plt.savefig(f"plots_separate_species/{sample_id}_pca_plot.png", bbox_inches='tight', bbox_extra_artists=[legend_pcr])
    plt.close()
    ...
    correlation_matrix = sp_group[selected_columns2].corr()

    # Plot the correlation matrix
    plt.figure(figsize=(12, 10))
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', vmin=-1, vmax=1)
    plt.title(f'{sample_id}: Correlation Matrix of Selected Features')
    plt.savefig(f"plots_separate_species/{sample_id}_correlation_matrix.pdf")
    plt.savefig(f"plots_separate_species/{sample_id}_correlation_matrix.png")
    plt.close()

    # Visualize pairwise relationships
    pairplot = sns.pairplot(sp_group[selected_columns2])
    pairplot.savefig(f'plots_separate_species/{sample_id}_pairwise_relationships.pdf')
    pairplot.savefig(f'plots_separate_species/{sample_id}_pairwise_relationships.png')
    plt.close()